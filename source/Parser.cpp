...
void Parser::ConvertParseToScriptItems()
{
	...
	for(uint i = 0; i < parse_funcs.size(); ++i)
	{
		...
		if(pf.type != V_VOID)
		{
			Type* type = GetType(pf.type);
			for(AnyFunction& f : type->funcs)
			{
				if(f.pf == &pf)
				{
					f = &sf;
...
ParseNode* Parser::ParseReturn()
{
	...
	else
	{
		...
		{
			ParseNode* r = ret->childs[0];
			if(r->result.type == V_REF || r->result.type == V_TYPE)
...
void Parser::ParseEnum(bool forward)
{
	...
	f->args.push_back(Arg(VarType(type->index, 0), 0, false));
	f->args.push_back(Arg(VarType(type->index, 0), 0, false));
	f->block = nullptr;
	f->flags = Function::F_BUILTIN;
	f->index = parse_funcs.size();
	f->module_proxy = &module;
	f->name = "$opAssign";
	f->node = nullptr;
	f->required_args = 2;
	f->result = V_BOOL;
...
// func_decl
CodeFunction* Parser::ParseFuncDecl(cstring decl, Type* type, bool builtin)
{
	assert(decl);

	CodeFunction* f = new CodeFunction;
	f->flags = (builtin ? Function::F_BUILTIN : Function::F_CODE);
	f->module_proxy = &module;
...
void Parser::ParseFuncInfo(Function* f, Type* type, bool in_cpp)
{
	...
	if(type && (f->special != SF_CTOR || !in_cpp) && !f->IsStatic())
	{
		...
		//		static methods
		//		code constructors
		f->args.insert(f->args.begin(), Arg(VarType((CoreVarType)type->index), 0, false));
		f->required_args++;
		if(in_cpp && !f->IsBuiltin())
			f->args.front().pass_by_ref = true;
	}
...
void Parser::ParseFunctionArgs(Function* f, bool in_cpp)
{
	assert(f);
	f->required_args = 0;
	...
	if(!t.IsSymbol(')'))
	{
		while(true)
		{
			...
			if(!in_cpp)
			{
				...
				current_function->arg_vars.push_back(arg);
			}
			else
			{
			...
			if(t.IsSymbol('='))
			{
				...
				{
				case PUSH_BOOL:
					f->args.push_back(Arg(item->bvalue));
					break;
				case PUSH_CHAR:
					f->args.push_back(Arg(item->cvalue));
					break;
				case PUSH_INT:
					f->args.push_back(Arg(item->value));
					break;
				case PUSH_FLOAT:
					f->args.push_back(Arg(item->fvalue));
					break;
				case PUSH_STRING:
					f->args.push_back(Arg(VarType(V_STRING), item->value, true));
					break;
				case PUSH_ENUM:
					f->args.push_back(Arg(VarType(item->result.type, 0), item->value, true));
					break;
				default:
					assert(0);
					break;
...
void Parser::ParseExprApplySymbol(vector<ParseNode*>& stack, SymbolNode& sn)
{
	...
	if(si.args == 1)
	{
		...
		else if(si.type == ST_SUBSCRIPT)
		{
			...
			vector<AnyFunction> funcs;
			ltype->FindAllFunctionOverloads(si.op_code, funcs);
			if(funcs.empty())
		...
		else if(si.type == ST_CALL)
		{
			...
			vector<AnyFunction> funcs;
			type->FindAllFunctionOverloads(si.op_code, funcs);
			if(funcs.empty())
	...
	else
	{
		...
		if(si.symbol == S_MEMBER_ACCESS)
		{
			...
			{
				vector<AnyFunction> funcs;
				type->FindAllFunctionOverloads(*right->str, funcs);
				if(type->index == V_TYPE)
					GetType(left->value)->FindAllStaticFunctionOverloads(*right->str, funcs);
				if(funcs.empty())
				{
					cstring err;
					if(type->index == V_TYPE)
...
ParseNode* Parser::ParseAssign(SymbolInfo& si, NodeRef& left, NodeRef& right)
{
	...
	if(si.op == NOP)
	{
		...
		vector<AnyFunction> funcs;
		left_type->FindAllFunctionOverloads(si.op_code, funcs);
		if(funcs.empty())
			t.Throw("Invalid types '%s' and '%s' for operation '%s'.", GetTypeName(left), GetTypeName(right), si.name);
...
void Parser::CheckFindItem(const string& id, bool is_func)
{
	...
	if(found_type != F_NONE && !(is_func && found_type == F_FUNC))
...
ParseVar* Parser::GetVar(ParseNode* node)
{
	...
	case PUSH_ARG_REF:
		...
		return current_function->arg_vars[node->value];
	default:
		assert(0);
		return nullptr;
	}
...
OpResult Parser::CanOp(SYMBOL symbol, SYMBOL real_symbol, ParseNode* lnode, ParseNode* rnode)
{
	...
	if((ltype->IsClass() || rtype->IsClass()) && symbol != S_IS)
	{
		...
		try
		{
			...
			if(builtin)
			{
				node->result = builtin.f->result;
				node->op = (Op)si.op;
				Cast(node->childs[1], builtin.f->args[1].vartype);
			}
...
CastResult Parser::MayCast(ParseNode* node, VarType vartype, bool pass_by_ref)
{
	...
	else
	{
		...
		{
			// find cast function
			result.cast_func = left->FindSpecialFunction(SF_CAST, [vartype](AnyFunction& f) {return f.f->result == vartype; });
			if(!result.cast_func && node->result.type == V_REF)
			{
				// find cast function when passed by reference
				result.cast_func = GetType(node->result.subtype)->FindSpecialFunction(SF_CAST, [vartype](AnyFunction& f) {return f.f->result == vartype; });
			}
		}

		// find ctor cast function
		result.ctor_func = right->FindSpecialFunction(SF_CTOR, [node](AnyFunction& f)
		{
			uint required = (f.IsCode() ? 1u : 2u);
			return f.f->args.size() == required
				&& f.f->args[required - 1].vartype == node->result
				&& f.f->IsImplicit();
		});
		if(!result.ctor_func && node->result.type == V_REF)
		{
			...
			result.ctor_func = right->FindSpecialFunction(SF_CTOR, [node](AnyFunction& f)
			{
				uint required = (f.IsCode() ? 1u : 2u);
				return f.f->args.size() == required
					&& f.f->args[required - 1].vartype.type == node->result.subtype
					&& f.f->IsImplicit();
			});
		}
...
void Parser::CopyFunctionChangedStructs()
{
	for(ParseFunction* pf : parse_funcs)
	{
		...
		for(ParseVar* arg : pf->arg_vars)
		{
			if(arg->mod && GetType(arg->vartype.type)->IsStruct())
			{
				if(!node)
				{
					node = ParseNode::Get();
				...
				node->push(COPY_ARG, arg->index);
			}
		}
		if(node)
			pf->node->childs.insert(pf->node->childs.begin(), node);
	}
}
...
void Parser::FindAllFunctionOverloads(const string& name, vector<AnyFunction>& items)
{
	...
	if(current_type)
	{
		for(AnyFunction& f : current_type->funcs)
		{
			if(f.IsParse() && f.f->name == name)
				items.push_back(f.pf);
		}
	}
}
...
VarType Parser::GetRequiredType(ParseNode* node, Arg& arg)
{
...
int Parser::MatchFunctionCall(ParseNode* node, Function& f, bool is_parse, bool obj_call)
{
	...
	if(node->childs.size() + offset - node_offset > f.args.size() || node->childs.size() + offset < f.required_args)
		return 0;
	...
	{
		Arg& arg = f.args[i + offset - node_offset];
...
AnyFunction Parser::ApplyFunctionCall(ParseNode* node, vector<AnyFunction>& funcs, Type* type, bool ctor, bool obj_call)
{
	...
	{
		int m = MatchFunctionCall(node, *f.cf, !f.IsCode(), obj_call);
		if(m == match_level)
			match.push_back(f);
	...
	{
		AnyFunction func = match.front();
		Function& f = *func.f;
		CheckFunctionIsDeleted(f);
		if(f.IsBuiltin())
			return func;
		bool callu_ctor = false;

		// remove push type from static method call or add pop
		if(f.IsStatic() && obj_call)
		{
		...
		if(f.special == SF_CTOR && !func.IsCode())
		{
			// user constructor call
			callu_ctor = true;
		}
		else if(current_type && f.type == current_type->index && !f.IsStatic())
		{
			// push this
			ParseNode* thi = ParseNode::Get();
			thi->op = PUSH_ARG;
			thi->result = (CoreVarType)f.type;
			thi->value = 0;
			thi->source = nullptr;
		...
		// fill default params
		for(uint i = node->childs.size() + (callu_ctor ? 1 : 0); i < f.args.size(); ++i)
		{
			Arg& arg = f.args[i];
		...
		else if(node->result.type == V_REF && f.IsCode())
		{
			ReturnStructVar* rsv = new ReturnStructVar;
			rsv->index = -1;
			rsv->node = node;
...
bool Parser::FindMatchingOverload(Function& f, BASIC_SYMBOL symbol)
{
	BasicSymbolInfo& bsi = basic_symbols[symbol];
	for(int i = 0; i < 3; ++i)
	{
		...
		if(s != S_INVALID)
		{
			...
			if(!si.op_code)
				continue;
			if(si.args == f.args.size() || s == S_SUBSCRIPT || s == S_CALL)
			{
				f.name = si.op_code;
				return true;
			}
...
ParseFunction* Parser::AnalyzeArgs(VarType result, SpecialFunction special, Type* type, cstring name, int flags)
{
	...
	func->flags = flags;
	func->module_proxy = &module;
	func->name = name;
	func->index = parse_funcs.size();
	func->required_args = 0;
	func->special = special;
	if(type && !IS_SET(flags, Function::F_STATIC))
	{
		func->args.push_back(Arg(VarType(type->index, 0), 0, false));
		func->required_args++;
	}
	...
	if(!t.IsSymbol(')'))
	{
		...
		while(true)
		{
			...
			if(t.IsSymbol('='))
			{
				prev_def = true;
				t.Next();
				func->args.push_back(Arg(vartype, 0, true));
				t.SkipToSymbol("),");
			}
			else
			{
...
int Parser::CreateDefaultFunctions(Type* type, int define_ctor)
{
	...
	// ctor
	AnyFunction f = type->FindSpecialFunction( SF_CTOR, [](AnyFunction& f) { return true; });
	...
	if(type->IsStruct())
	{
		f = type->FindSpecialFunction(SF_CTOR, [vartype](AnyFunction& f)
		{
			uint offset = (f.f->IsCode() ? 0 : 1);
			return f.f->args.size() == offset + 1 && In(f.f->args[offset].vartype, { vartype, VarType(V_REF, vartype.type) });
		});
		if(!f)
		{
			// decl
			...
			func->args.push_back(Arg(vartype, 0, false));
			func->block = nullptr;
			func->flags = Function::F_DEFAULT | Function::F_BUILTIN;
			func->index = parse_funcs.size();
			func->type = type->index;
	...
	f = type->FindFunction(info->op_code, [type](AnyFunction& f) { return f.f->args[1].vartype.type == type->index; });
	if(!f)
	{
...
void Parser::VerifyFunctionsDefaultArguments()
{
	...
	for(ParseFunction* pf : parse_funcs)
	{
		if(pf->required_args == pf->args.size() || pf->IsDefault())
			continue;
		AnalyzeArgsDefaultValues(pf);
	}
}

void Parser::AnalyzeArgsDefaultValues(ParseFunction* pf)
{
	t.MoveTo(pf->start_pos);
	uint offset = (pf->special == SF_CTOR ? 1u : 0u);

	for(uint i = offset, count = pf->args.size(); i <= count; ++i)
	{
		VarType vartype = AnalyzeVarType();
		t.AssertItem();
		t.Next();
